...
while True:
    ...
    with open(f'messages_from_Artur_for_test.csv', newline='') as File:
        #reader_1 = csv.reader(File)
        test_cache = []
        lines = File.readlines()
        for row_check in lines[-10:]:
                        ...
                        if float(usdt_info['cashBal']) >= min_cost_for_order and float(usdt_info['cashBal']) >= ((float(min_price_for_order))*2): #проверяю что балан больше минимальной цена которую мы выставили для торгов, и проверял что баланс больше мин цена вхождения Х2
                            if min_price_for_order < (min_cost_for_order/2): # сравнение выше с ценой мин ставки которую ты установил.
                                ...
                                for price_order in price:
                                    print(price_order, float(input_cost[num_tp_trigger]))
                                    if price_order == float(input_cost[num_tp_trigger]):
                                        ...
                                        money_for_algo_SL = (((current_cost_for_order * min_price_for_order)/(len(TP_for_order))) / float(price_low))
                                        ...
                                        if result_order["code"] == "0":
                                            ...
                                            check_point = 1
                                            with open(f'messages_from_Artur_for_test.csv', 'a', newline='', encoding="utf-8") as csvfile:
                                            ...
                                            check_point = 1
                                            with open(f'messages_from_Artur_for_test.csv', 'a', newline='',
