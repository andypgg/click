        ...
        for row in reader:
            ...
            if take_order != 1:
                    ...
                    if check_coin1 != None:
                        ...
                        if float(usdt_info['cashBal']) >= min_cost_for_order and float(usdt_info['cashBal']) >= ((float(min_price_for_order))*2): #проверяю что балан больше минимальной цена которую мы выставили для торгов, и проверял что баланс больше мин цена вхождения Х2
                            if min_price_for_order < (min_cost_for_order/2): # сравнение выше с ценой мин ставки которую ты установил.
                                ...
                                check_for_skip = 0
                                if price != None:
                                    for price_order in sorted(price):
                                        if price_order == float(input_cost[num_tp_trigger]):
                                            ...
                                            if result_order["code"] == "0":
                                                ...
                                                message_tg = (
                                                f"Successful order request，order_id = {result_order['data'][0]['ordId']} {check_coin1} {side_to_trade} {posSide_id} {SL_calculate} {formatted_number_SL} {price_order}")
                ...
                for test_row1 in test_cache:
                    # print(test_row)
                    if str(message_id) in str(test_row1) and str(coin) in str(test_row1):
                        coin_found_here = 1
                        break
                if coin_found_here == 0:
